...
use Cake\I18n\Time;
use App\View\Helper\ShopHelper;
class AppController extends Controller
{
    ...
    public function beforeFilter(Event $event)
    {
		...
		////// package fetch from table and make constants start //////////
		$this->loadModel('Packages');
		$query = $this->Packages->find();
		$packages = $query->toArray();
		foreach($packages as $package)
		{
			define(strtoupper(preg_replace('/\s+/', '_', $package->name)), $package->id);
			strtoupper(preg_replace('/\s+/', '_', $package->name));
		}
		////// package fetch from table and make constants end //////////
    }
