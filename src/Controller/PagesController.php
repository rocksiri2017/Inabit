...
class PagesController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['home', 'terms_conditions', 'privacy_policy', 'gcron', 'about']);
    }
    ...
    public function home($arg) {
        ...
        // Today Orders
        $count_today_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ->bind(':start', new \DateTime($today_string_date), 'date')
        ->bind(':end',   new \DateTime($today_string_date), 'date')
        ->count());
		
        // This Week Orders
        $count_week_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ...
        // This Month Orders
        $count_month_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ...
        // New Orders
        $count_new_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ...
        // Completed
        $count_completed_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ...
        // This Year Orders
        $count_year_orders = Number::format($this->Orders
        ->find()
        ->where([
            'created BETWEEN :start AND :end'
        ])
        ...
        ->count());
         ...
         $this->set(compact('count_today_orders', 'count_week_orders', 'count_month_orders', 'count_new_orders', 'count_completed_orders', 'count_year_orders'));
    ...
    public function gcron() {
        ...
        ->emailFormat('html')
        ->from(['info@inabit.com.au' => '[Auto Daily] Contractors Submission'])
