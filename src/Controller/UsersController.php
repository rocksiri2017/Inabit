    ...
    public function register() {
        ...
        if ($this->request->is('post')) {
            ...
            if (isset($this->request->type) && $this->request->type == 'merchant') {
				...
				########## Start by K.V Dev ############
				/* $userRegister->package_id = 10;
				$userRegister->is_trial_package = 1;
				$userRegister->package_end_date = date('Y-m-d',time()+2592000); // after 30 days date
				//$userRegister->package_end_date = date('Y-m-d'); // after 30 days date */
				########## End by K.V Dev ############
            ...
            if ($this->Users->save($userRegister)) {
				...
				if (isset($this->request->type) && $this->request->type == 'merchant') {
					...
					$queryPackages = $this->UserPackages->find()->where(['user_id' => $userRegister->id]);
					$packageRowCount = $queryPackages->count();
					if($packageRowCount == 0){
					
						$userPackageNewData = $this->UserPackages->newEntity();
						$userPackageNewData->user_id = $userRegister->id;
						$userPackageNewData->is_trial_package = 1;
						$userPackageNewData->package_id = STARTER;
						$userPackageNewData->trial_package_end_date = date('Y-m-d',time()+2592000); // after 30 days date
                ...
                $session ->write('login_user_id',$user->id);
    ...
    public function login()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($userLogin) {
                ...
                $logged_user = $user->first_name;

                $session = $this->request->session();
                $session->write("login_user_id",$user->id);
                $url = ['controller' => 'pages', 'action' => 'home',$logged_user];
    ...
    public function forgotPassword()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $email->profile('default')
                ...
                ->emailFormat('html')
                ->from(['info@inabit.com.au' => '[INABIT] Forgot your Password'])
                ->to($user->email)
                ->subject('[INABIT] Forgot your Password')
                ->viewVars($viewVars)
                ->send();
            $this->Flash->success("An E-mail has been sent to <strong>" . $user->email . "</strong>. Please follow the instructions in the E-mail.");
        }
        $this->set(compact('user'));
    }
    /**
