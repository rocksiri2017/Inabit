      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12">	
			...
			<?php 
				...
				list($width, $height, $type, $attr) = getimagesize(WWW_ROOT.$product->hero); 
				
				if($width < "200"){			
					echo $this->Html->image('http://' . $_SERVER['HTTP_HOST'].'/timthumb.php?src='.$product->hero.'&w=320&h=520&a=t',['class' => 'img-responsive', 'style' => 'margin: 0px auto;']);			
				}else{			
					echo $this->Html->image($product->hero,['class' => 'img-responsive', 'style' => 'margin: 0px auto;']);			
				}			
			 ?>
        ...
        <div class="row product-general">
          <div class="col-md-12">
           ...
           <?php /*  <p class="text-left blue-color"><strong>$<?= number_format((float)$product->price, 2, '.', ''); ?></strong></p>  */?>            
          </div>
        </div>		
        <div class="row">
          <div class="col-md-12 content-product">
            <p class="black-color"><?= ucwords($product->content); ?></p>
          </div>
        </div>		
			<div class="row addToCartContainer mgt-sm">
			  <div class="col-md-12 col-sm-12 col-xs-12">
				...
				<div class="col-md-9 col-sm-9 col-xs-9">
				 <?php /*  <button class="mbr-buttons__btn btn btn-lg btn-info btn-green-transparent addToCart" data-url="<?php echo $this->Url->build(['controller' => 'cart', 'action' => 'add_cart',$product->id]); ?>">Add To List</button>
				   */?>
					 <a class="mbr-buttons__btn btn btn-lg btn-info btn-green-transparent" style="padding:13px" href="<?php echo $this->Url->build(['controller' => 'search', 'action' => 'advanced',$product->id]); ?>">Add To List</a>
				</div>
			  </div>
			</div>
			<div class="row">
			...
			<?php if($feedbacks->toArray()): ?>  
			<div class="row product-general">
				<div class="col-md-12">
					<p class="text-left blue-color"><strong><a style="color:#328ca9;" href="<?php echo $this->Url->build(['controller' => 'products', 'action' => 'feedback',$product->id]); ?>">Feedback</a></strong></p>
				</div>
			</div>
			<?php endif; ?>
		<div class="row">
		...
		<tbody>
			<?php foreach($feedbacks as $value):  ?>
				...
				<tr class='clickable-row' data-href="<?= $this->Url->build(['controller' => 'shop', 'action' => 'index', $value->id, 'prefix' => false]) ?>">
					...
					<td>
						<?php echo ucwords($value['user']['first_name'].' '.$value['user']['last_name']); ?>
					</td>
