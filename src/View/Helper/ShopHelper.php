...
class ShopHelper extends Helper
{
	...
	function countProductPrice($shop_id){ 
		$this->OrderProduct = TableRegistry::get('OrderProducts');	
		$conditions = ['OrderProducts.store_id' => $shop_id];
        $count_product = $this->OrderProduct
            ->find()			
			 ->where([$conditions]);         
			...
			foreach($count_product as $value){
				...
				$count_products += $value->price;				
			}
			// pr($count_products);die;
    ...
    function shopWeight($shop_id){
        $this->Products = TableRegistry::get('Products');
        $count_product = $this->Products
            ->find()
            ->where([
                'Products.shop_id' => $shop_id,'Products.status' => 1
            ]);
        $total = 0;
        foreach($count_product as $value){
            $total += $value->weight;
    ...
    function shopDimension($shop_id){
        $this->Products = TableRegistry::get('Products');
        $count_product = $this->Products
            ->find()
            ->where([
